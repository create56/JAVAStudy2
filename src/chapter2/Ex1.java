...
public class Ex1 {
	public static void main(String[] args) {
		...
		Date date = new Date();
		// 자바1.1버전에서 추가된 날짜와 시간을 핸들링하는 클래스
		// Calendar 클래스는 추상 클래스이므로 인스턴스를 만들 수없음
		// 대신 Calendar 클래스에 있는 static 메서드인 getInstance를 사용해서
		// Calendar 클래스의 자식 클래스 인스턴스는 만들 수 있음
		// 프로그램이 구동되는 시스템의 국가와 지역 설정을 확인해서
		// 해당 시스템에 맞는 날짜를 핸들링 할 수있는 클래스의 인스턴스가 반환된다.
		// 그레고리력(태국외 다른 나라), 불력(태국) 
		Calendar calendar = Calendar.getInstance();
		
		
		// Data 클래스의 인스턴스 -> Calendar 클래스의 인스턴스로 변환
